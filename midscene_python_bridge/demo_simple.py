...
async def save_screenshot(agent: AgentOverChromeBridge, save_dir: str = "screenshots"):
    """保存页面截图"""
    try:
        Path(save_dir).mkdir(parents=True, exist_ok=True)
        screenshot_data = await agent.take_screenshot()
        ...
        logger.error(f"保存截图时发生错误: {e}", exc_info=True)
        return None
...
async def main():
    ...
    try:
        ...
        if agent.connected:
            ...
            # 4. 执行JavaScript获取页面元信息 (使用JSON.stringify)
            js_get_meta = "JSON.stringify(Array.from(document.querySelectorAll('meta')).map(m => ({ name: m.name, content: m.content })))"
            logger.info(f"执行JS (获取meta): {js_get_meta[:100]}...")
            meta_json_str = await agent.evaluate(js_get_meta)
            if meta_json_str and isinstance(meta_json_str, str):
                try:
                    meta_info = json.loads(meta_json_str)
                    logger.info(f"页面元信息 (解析后): {meta_info}")
                except json.JSONDecodeError:
                    logger.error(f"解析元信息JSON失败: {meta_json_str}")
            else:
                logger.info(f"页面元信息 (原始返回): {meta_json_str}")

            # 5. 获取页面HTML内容
            try:
                html_content = await agent.get_content()
                logger.info(f"页面HTML内容长度: {len(html_content)} 字符")
            except Exception as e:
                logger.error(f"获取页面内容失败: {e}")

            # 示例：截取页面截图
            try:
                screenshot_path = await save_screenshot(agent)
                if screenshot_path:
                    logger.info(f"成功保存页面截图到: {screenshot_path}")
            except Exception as e:
